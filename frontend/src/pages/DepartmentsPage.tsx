...
export const DepartmentsPage: React.FC<DepartmentsPageProps> = ({
  ...
  // Filter projects belonging to the active department category
  const deptProjects = projects.filter(p => {
    const matchesCategory = p.category.toLowerCase().includes(activeDept.category.toLowerCase()) ||
  ...
  // Calculate department aggregate metrics
    ...
    <div className="space-y-6 pb-20">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-6 border-b border-amber-100">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
              Departmental Monitoring Portals
            </h1>
            <p className="text-xs sm:text-sm text-slate-600 mt-1 max-w-3xl">
              Sector-specific intelligence dashboards configured with Indian administrative benchmarks, 
              nodal executing agencies, and automated project milestone tracking.
            </p>
          </div>
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-xs relative overflow-hidden">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-slate-100">
          ...
          <div className="flex items-center gap-2 self-start md:self-auto bg-slate-50 p-2.5 rounded-xl border border-slate-200 text-xs">
            ...
            <span className="text-slate-600">Standard: <strong>{activeDept.benchmark}</strong></span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mt-6 text-xs">
          <div className="p-4 bg-slate-50/80 rounded-2xl border border-slate-200/80">
            <span className="text-slate-500 text-[10px] font-mono font-bold uppercase block">Total Sanctioned</span>
            <span className="text-lg font-black text-slate-900 mt-1 block">
              {formatIndianCurrency(totalSanctioned)}
            </span>
            <span className="text-[10px] text-slate-400 font-mono">{allDeptProjects.length} schemes tracked</span>
          </div>

          <div className="p-4 bg-slate-50/80 rounded-2xl border border-slate-200/80">
            <span className="text-slate-500 text-[10px] font-mono font-bold uppercase block">Disbursed Outlay</span>
            <span className="text-lg font-black text-slate-900 mt-1 block">
              {formatIndianCurrency(totalSpent)}
            </span>
            <span className="text-[10px] text-emerald-600 font-bold font-mono">
              {totalSanctioned > 0 ? Math.round((totalSpent / totalSanctioned) * 100) : 0}% fund utilization
            </span>
          </div>

          <div className="p-4 bg-slate-50/80 rounded-2xl border border-slate-200/80">
            <span className="text-slate-500 text-[10px] font-mono font-bold uppercase block">Avg Physical Velocity</span>
            <span className="text-lg font-black text-emerald-600 mt-1 block">
              {avgPhysical}%
            </span>
            <span className="text-[10px] text-slate-400">MB Record verified</span>
          </div>

          <div className="p-4 bg-emerald-50/70 rounded-2xl border border-emerald-200/80">
            <span className="text-emerald-800 text-[10px] font-mono font-bold uppercase block">Completed & Handover</span>
            <span className="text-lg font-black text-emerald-900 mt-1 block">
              {completedCount} Works
            </span>
            <span className="text-[10px] text-emerald-700 font-semibold">100% UC generated</span>
          </div>

          <div className="p-4 bg-indigo-50/70 rounded-2xl border border-indigo-200/80">
            <span className="text-indigo-800 text-[10px] font-mono font-bold uppercase block">Under Execution</span>
            <span className="text-lg font-black text-indigo-900 mt-1 block">
              {inProgressCount} Works
            </span>
            <span className="text-[10px] text-indigo-700 font-semibold">Under surveillance</span>
          </div>

          <div className="p-4 bg-rose-50/70 rounded-2xl border border-rose-200/80">
            <span className="text-rose-800 text-[10px] font-mono font-bold uppercase block">Watchlist Warnings</span>
            <span className="text-lg font-black text-rose-900 mt-1 block">
              {highRiskCount + stalledCount} Works
            </span>
            <span className="text-[10px] text-rose-700 font-bold">Needs verification audit</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gov-ivory-border p-4 shadow-gov flex flex-col md:flex-row items-center justify-between gap-3">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            placeholder={`Search ${activeDept.category} works by Title, ID, District, or Contractor...`}
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <span className="text-xs font-bold text-slate-500 shrink-0">Execution Status:</span>
          <select
            value={statusFilter}
          ...
          >
            ...
            <option value="STALLED">Stalled / Stagnated</option>
          </select>
        </div>
      </div>
      ...
      {loading ? (
        <div className="p-12 text-center bg-white rounded-xl border border-slate-200">
          ...
          <p className="text-xs font-semibold text-gov-navy">Retrieving {activeDept.category} project ledgers...</p>
        </div>
      ) : deptProjects.length === 0 ? (
        <div className="p-12 text-center bg-white rounded-xl border border-slate-200">
          <activeDept.icon className="w-12 h-12 text-slate-300 mx-auto mb-3" />
          <h3 className="text-sm font-bold text-slate-700">No works matched current search criteria</h3>
          <p className="text-xs text-slate-400 mt-1">Try clearing the search query or changing the execution status filter.</p>
          <button
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-500 mt-2.5 space-y-1">
                    ...
                    <p className="text-[11px] text-slate-400 pl-5">
                      Executing Agency: <span className="font-medium text-slate-700">{proj.contractor_name || 'Designated Nodal Agency'}</span>
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 my-4 p-3.5 bg-slate-50/90 rounded-xl border border-slate-100 text-xs">
                    <div>
                      <span className="text-slate-400 text-[10px] uppercase font-mono font-bold block">Sanctioned</span>
                    ...
                    <div>
                      <span className="text-slate-400 text-[10px] uppercase font-mono font-bold block">Status</span>
                    ...
                    <div>
                      <span className="text-slate-400 text-[10px] uppercase font-mono font-bold block">Physical</span>
                    ...
                    <div>
                      <span className="text-slate-400 text-[10px] uppercase font-mono font-bold block">Disbursed</span>
                  ...
                  <div className="space-y-1 mb-3">
                    ...
                    {delayDays > 0 && (
                      <span className="text-[10px] text-rose-600 font-bold block">
                        ⚠ {delayDays} days schedule slippage
                      </span>
                    )}
                  </div>
                </div>
